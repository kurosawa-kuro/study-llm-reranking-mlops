...
| [`04_運用.md`](./04_運用.md) | **環境構築 (STEP 1–13)** + 定常運用 + インシデント対応 + ロールバック | 新規参加者 / 運用担当 | 手順・閾値・アラート変更時 |
...
| 環境構築手順の変更 (STEP の増減・ツール変更) | `04_運用.md §1` + `Makefile (doctor / bootstrap)` |
...
- **コマンドは `make` ターゲット優先**。生 `gcloud` / `bq` / `terraform` は動的引数 (revision 名・イメージタグ・トークン値) が必要なときに限り使う
...
- `04_運用.md §1` (環境構築) は **上から順に叩いて完走することが品質基準**。STEP を追加する場合は依存関係を壊していないか実測で確認する
