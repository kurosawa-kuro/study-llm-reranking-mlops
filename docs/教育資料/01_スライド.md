...
| 前講義で見たもの | 今回どこで使うか | 担当技術 |
|---|---|---|
| 特徴量 | 物件属性、行動特徴、`me5_score` | BigQuery / LightGBM |
| 推論 | `/search` 実行時の順位計算 | FastAPI / LightGBM |
| 前処理済み入力 | フィルタ条件、埋め込み生成対象 | Meilisearch / ME5 |
| モデル | 最終順位を決める再ランキング器 | LightGBM |

> 前講義は「1つの予測モデルを作る話」、今回は「検索の中で複数技術をつなぐ話」です。
...
### それぞれの強み
- `Meilisearch`: Elasticsearch の代替として使いやすく、高速、厳密条件に強い、実装が単純
...
> まずは Meilisearch が担当する「速く拾う段」を見ます。
...
> 技術の主役が Meilisearch から ME5 に切り替わります。
...
> 以降は「近い文を探す」ではなく「事業上よい順に並べる」話です。
